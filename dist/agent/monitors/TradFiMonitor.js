...
exports.TradFiMonitor = void 0;
const axios_1 = __importDefault(require("axios"));
const OnChainMonitor_1 = require("./OnChainMonitor");
const types_1 = require("./types");
class TradFiMonitor extends OnChainMonitor_1.BaseMonitor {
    constructor(config) {
        super(config);
        // Stock ticker to crypto token mappings
        this.TICKER_MAPPINGS = [
            {
                ticker: 'NVDA',
                cryptoTokens: [
        ...
        this.secClient = axios_1.default.create({
            timeout: 15000,
            headers: {
     ...
     */
    async checkEarnings(mapping) {
        const signals = [];
        const cacheKey = `earnings_${mapping.ticker}`;
        try {
            // Check cache first
            let earningsData = this.getCached(cacheKey);
            if (!earningsData) {
            ...
            if (epsMissPercent > 10) {
                for (const token of mapping.cryptoTokens) {
                    signals.push({
                        type: types_1.SignalType.EARNINGS_MISS,
                        token: token.address,
                        chain: token.chain,
            ...
            if (revenueMissPercent > 5) {
                for (const token of mapping.cryptoTokens) {
                    signals.push({
                        type: types_1.SignalType.REVENUE_MISS,
                        token: token.address,
                        chain: token.chain,
     ...
     */
    async checkInsiderTrading(mapping) {
        const signals = [];
        const cacheKey = `insider_${mapping.ticker}`;
        try {
            // Check cache first
            let transactions = this.getCached(cacheKey);
            if (!transactions) {
            ...
            if (largeInsiderSells.length > 0 || coordinatedSells) {
                for (const token of mapping.cryptoTokens) {
                    ...
                    if (score > 0) {
                        signals.push({
                            type: types_1.SignalType.INSIDER_SELLING_SEC,
                            token: token.address,
                            chain: token.chain,
                            score,
                            metadata,
    ...
    async fetchEarningsData(ticker) {
        try {
            ...
            const response = await this.httpClient.get(`https://query2.finance.yahoo.com/v10/finance/quoteSummary/${ticker}`, {
                params: {
     ...
     */
    async fetchInsiderTradings(ticker) {
        try {
            ...
            const response = await this.secClient.get(`https://data.sec.gov/submissions/CIK${cik.padStart(10, '0')}.json`);
    ...
    async getCIK(ticker) {
        ...
        let cik = this.getCached(cacheKey);
        if (cik)
            return cik;
        try {
            // SEC provides a ticker to CIK mapping
            const response = await this.secClient.get('https://www.sec.gov/files/company_tickers.json');
     ...
     */
    async parseForm4(cik, accessionNumber) {
        try {
     ...
     */
    detectLargeInsiderSells(transactions) {
        return transactions.filter(t => {
            const isCLevel = t.title.match(/CEO|CFO|Chief/i);
            const isLargeSell = t.transactionType === 'SELL' && t.totalValue > 5000000;
     ...
     */
    detectCoordinatedSells(transactions) {
...
exports.TradFiMonitor = TradFiMonitor;
